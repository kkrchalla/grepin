#!/usr/bin/perl -w
#$rcs = ' $Id: deldbfls.pl,v 1.00 2004/08/12 22:45:42 Grepin Exp $ ' ;
...
my $title = "Deletion of all the database files";
...
print "   Using $db_package...\n";
  ...
  delete_db($LATEST_IDS_DB_FILE); 
  delete_db($SESSION_DB_FILE); 
  delete_db($USER_PWD_DB_FILE); 
  delete_db($USER_PROFILE_DB_FILE); 
  delete_db($USER_INDEX_DATA_DB_FILE); 
  delete_db($USER_TEMPLATE_DATA_DB_FILE); 
  delete_db($USER_STATUS_DB_FILE); 
  delete_db($JOB_STATUS_DB_FILE); 
  delete_db($QUEUE_DB_FILE); 
  delete_db($INDEX_QUEUE_DB_FILE); 
  delete_db($REPORT_QUEUE_DB_FILE); 
  delete_db($SEARCHLOG_DB_FILE); 
  delete_db($NOMATCHLOG_DB_FILE); 
  delete_db($AMZN_RSLT_DB_FILE); 

  delete_db($AFF_PWD_DB_FILE);
  delete_db($AFF_PROFILE_DB_FILE);
  delete_db($AFF_USER_DB_FILE);
  delete_db($USER_AFF_DB_FILE);
  delete_db($TOP_BAR_HTML_DB_FILE);
  delete_db($BOT_BAR_HTML_DB_FILE);
  delete_db($LEFT_BAR_HTML_DB_FILE);

print "   Deleting dbfiles done.\n";
exit;
...
sub delete_db {
  my $name = shift;
  if ((unlink $name)) {
    print " deleted db file - $name\n";
  } else {
    print " **** error - could not delete file - $name\n";
  }
}
